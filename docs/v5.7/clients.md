...
- [配置文件选项](/v5.7/clients.html#_4-配置文件选项)
...
|登录流程发生了几次？|通过定义的 [Authenticator](/v5.7/authenticators.html) 和 `ProfileCreator` 对每个 HTTP 请求（在“安全过滤器”中）进行认证。<br/>出于性能原因，可以通过将当前 `Authenticator` 包装在 `LocalCachingAuthenticator` 中来使用缓存，或者可以配置“安全过滤器”以在会话中保存配置文件（`ProfileStorageDecision`）|认证仅发生一次（在“回调过滤器”中）|
