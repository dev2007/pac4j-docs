...
Kerberos 客户端需要定义[认证器](/v5.6/authenticators.html)来处理凭据校验。最可能的是，你只需要将现有的 `KerberosAuthenticator` 与 `SunJaasKerberosTicketValidator` 一起使用，即可完成 Kerberos 票证验证的所有繁重工作。
...
> [原文链接](https://www.pac4j.org/5.6.x/docs/clients/kerberos.html)
