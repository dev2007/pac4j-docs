...
客户端代表一种认证机制。它执行登录流程并返回（如果成功）用户配置文件。大量客户端可用于：

- [OAuth 协议](/v4.0/clients/oauth.html)
- [SAML 协议](/v4.0/clients/saml.html)
- [CAS 协议](/v4.0/clients/cas.html)
- [OpenID Connect 协议](/v4.0/clients/openid-connect.html)
- [HTTP 协议](/v4.0/clients/http.html)
- [OpenID 协议](/v4.0/clients/openid.html)
- [谷歌应用引擎（Google App Engine）支持](/v4.0/clients/google-app-engine.html)
- [Kerberos (SPNEGO Negotiate)协议](/v4.0/clients/kerberos.html)

虽然大多数客户端都是独立可用的，但 HTTP 客户端需要定义一个[认证器（Authenticator）](/v4.0/authenticators.html) 来处理凭证校验。

客户端（如授权器）通常在[安全配置（security configuration）](/v4.0/config.html)中定义。

每个客户端都有一个以类名作为默认的名称（如 `FacebookClient`），但可以使用 `setName` 方法将其显式设置为另一个值。

理解以下主要特性：

- [直接客户端与间接客户端](/v4.0/clients.html#_1-直接客户端-vs-间接客户端)
- [计算角色和权限](/v4.0/clients.html#_2-计算角色和权限)
- [回调 URL](/v4.0/clients.html#_3-回调-url)
- [配置文件定义](/v4.0/clients.html#_4-配置文件选项)
- [AJAX 请求](/v4.0/clients.html#_5-ajax-请求)
- [Client 方法](/v4.0/clients.html#_6-client-方法)
- [原始请求的 URL](/v4.0/clients.html#_7-原始请求的-url)
- [静默登录](/v4.0/clients.html#_8-静默登录)

## 1）直接客户端 vs 间接客户端

客户端有两种：直接的，用于 web 服务身份验证；间接的，用于 UI 身份验证。

以下是他们的行为和差异：

||直接客户端=web 服务认证|间接客户端=UI 认证|
|--|--|--|
|[认证流程](/v4.0/authentication-flows.html)|1）为每个 HTTP 请求传递凭据（传递给 “[安全过滤器](/v4.0/how-to-implement-pac4j-for-a-new-framework.html)”）|1） 原始请求的 URL 保存在会话中（通过“安全过滤器”）<br/> 2） 用户被重定向到身份提供者（identity provider）（通过“安全过滤器”）<br/> 3） 认证发生在身份提供者（`identity provider`）（或本地的 `FormClient` 和 `IndirectBasicAuthClient`）<br/> 4） 用户被重定向回调端点/URL（“回调过滤器”）<br/> 5） 用户被重定向到原始请求的 URL（通过“[回调过滤器](/v4.0/how-to-implement-pac4j-for-a-new-framework.html)”）|
|登录流程发生了几次？|通过定义的 [Authenticator](/v4.0/authenticators.html) 和 `ProfileCreator` 对每个 HTTP 请求（在“安全过滤器”中）进行认证。<br/>出于性能原因，可以通过将当前 `Authenticator` 包装在 `LocalCachingAuthenticator` 中来使用缓存，或者可以使用可用的 web 上下文和 `ProfileManager` 类（由 `Authenticator` 或 `ProfileCreator`）将用户配置文件保存到 web 会话中|认证仅发生一次（在“回调过滤器”中）|
|默认情况下，用户配置文件保存在哪？|在 HTTP request 中（无状态）|在 web session 中（有状态）|
|凭据在哪里？|为每个 HTTP request 传递（由“安全过滤器”处理）|在认证提供者（`identity provider`）返回的回调端点上（由“回调过滤器”获取）|
|凭证是强制性的吗？|通常不会。如果没有提供凭据，直接客户端将被忽略（通过“安全过滤器”）|
...
并且你可以使用 `addAuthorizationGenerator` 方法添加任意数量的授权生成器，也可以使用 `setAuthorizationGenerators` 方法添加授权生成器列表。

事实上，`AuthorizationGenerator` 组件不仅可以用于计算角色和权限，还可以用于基于表单的记住我复选框定义配置文件的记住我的性质（请参阅： [RememberMeAuthorizationGenerator](https://github.com/pac4j/pac4j/blob/master/pac4j-http/src/main/java/org/pac4j/http/authorization/generator/RememberMeAuthorizationGenerator.java)）。

## 3）回调 URL

对于间接客户端，可以定义将在登录流程中使用的回调 URL：成功登录后，身份提供者（identity provider）将用户重定向到回调 URL 的应用程序。
...
在登录流程结束时，返回的用户配置文件由（内部）`Authenticator` 或 `ProfileCreator` 创建，后者保存 [配置文件定义](/v4.0/user-profile.html)。
...
|方法|用途|
|--|--|
|`Optional<RedirectionAction> getRedirectionAction(WebContext context)`（仅用于间接客户端）|它返回重定向操作，将用户重定向到身份提供者进行登录。<br/>通过 [RedirectionActionBuilder](https://github.com/pac4j/pac4j/blob/master/pac4j-core/src/main/java/org/pac4j/core/redirect/RedirectionActionBuilder.java) 在内部计算用户到身份提供者的重定向|
|`Optional<C> getCredentials(WebContext context)`|它从 HTTP 请求中提取凭据并验证它们。
...
|`Optional<UserProfile> getUserProfile(C credentials, WebContext context`|它构建经过身份验证的用户配置文件。<br/> 已验证用户配置文件的创建由 [ProfileCreator](https://github.com/pac4j/pac4j/blob/master/pac4j-core/src/main/java/org/pac4j/core/profile/creator/ProfileCreator.java) 执行|
|`Optional<UserProfile> renewUserProfile(UserProfile profile, WebContext context)`|它返回更新的用户配置文件|
|`Optional<RedirectionAction> getLogoutAction(WebContext context, UserProfile currentProfile, String targetUrl)`|它返回重定向操作以调用身份提供者注销。<br/>注销重定向操作计算由 [LogoutActionBuilder](https://github.com/pac4j/pac4j/blob/master/pac4j-core/src/main/java/org/pac4j/core/logout/LogoutActionBuilder.java) 完成|

客户端通常使用默认子组件填充：`RedirectionActionBuilder`、`CredentialExtractor`、`ProfileCreator`、`LogoutActionBuilder` 和 `Authenticator`，但必须定义 `Authenticator` 的 HTTP 客户端除外。当然，子组件还可以针对各种[定制](/v4.0/customizations.html)进行更改。

## 7）原始请求的 URL

原始请求的 URL 是在经过认证的过程开始之前调用的 URL：它在登录过程完成后从回调 URL 恢复。
...
> [原文链接](https://www.pac4j.org/4.0.x/docs/clients.html)
