...
[HTTP 客户端](/v4.0/clients/http.html)需要[认证器](https://github.com/pac4j/pac4j/blob/master/pac4j-core/src/main/java/org/pac4j/core/credentials/authenticator/Authenticator.java)来验证凭据。

此 `Authenticator` 接口只有一个方法：`void validate(C credentials, WebContext context) throws HttpAction, CredentialsException`。
...
- [LDAP](/v4.0/authenticators/ldap.html)
- [SQL](/v4.0/authenticators/sql.html)
- [JWT](/v4.0/authenticators/jwt.html)
- [MongoDB](/v4.0/authenticators/mongodb.html)
- [CouchDB](/v4.0/authenticators/couchdb.html)
- [IP address](/v4.0/authenticators/ip.html)
- [REST API](/v4.0/authenticators/rest.html)
...
默认情况下，`LocalCachingAuthenticator` 使用 Guava 作为其内部[存储](/v4.0/store.html)，但你可以通过 `setStore` 方法提供自己的存储。
...
因此，即使提供特定的 `ProfileCreator` 是完全可行的，它也可以开箱即用。

> [原文链接](https://www.pac4j.org/4.0.x/docs/authenticators.html)
